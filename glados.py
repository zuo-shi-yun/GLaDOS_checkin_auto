...
if __name__ == '__main__':
    ...
    for cookie in cookies:
        ...
        if 'message' in checkin.text:
            ...
            requests.get(
                'http://www.pushplus.plus/send?token=' + sckey + '&content=' + email + 'cookie已失效' + "&channel=mail")
            print('cookie已失效')  # 日志输出
    # --------------------------------------------------------------------------------------------------------#
    if sckey != "":
        url = 'http://www.pushplus.plus/send?token=' + sckey + '&title=' + email + '签到成功' + "&channel=mail" + '&content=' + sendContent
        print(url)
        requests.get(url)
        print('完成发送邮件')
